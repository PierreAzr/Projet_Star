...
class RelationEntrepriseController extends Controller
{
    ...
    {   
        ...
        // Recuperation de la table periode et trouver la periode qui correspond a la date voulu (date du jour par defaut) ainsi que la periode n-1
         ...
         if (!isset($date)) {
            echo("ifssettttttttttttttttt");
            $date = $request->get('date');
            echo("{$date}ttttttttttttttttt");
         }
         //$date = $request->get('date');
...
/* 
 ...
 /*        if (empty($tableau_complet) || empty($final_tab) || empty($total_tab)) {
            return redirect()->route('relation_entreprise_index')->with('flash_message', "La date corespond a une periode qui existe mais une des requete est vide")
            ->with('flash_type', 'alert-danger');   
        } */
     ...
     /*        //tableau groupes
        ...
        if (isset($prospects_tab) && $apprenants_tab) {
            ...
            $tableau_complet = array_merge($apprenants_tab);
        }elseif (isset($prospects_tab)){
            $tableau_complet = array_merge($prospects_tab);
        }else{
            return redirect()->route('relation_entreprise_index')->with('flash_message', "La date correspond a une periode qui existe mais une des requetes est vide")
            ->with('flash_type', 'alert-danger');  
        }
